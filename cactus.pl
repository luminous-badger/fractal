...
## Cactus fractal is Z*(n+1) = Z**3 + ( Z - 1 ) * Z - Z
## From: http://mathworld.wolfram.com/CactusFractal.html
...
## Cactus fractal is Z*(n+1) = Z**3 + ( Z - 1 ) * Z - Z
		...
		while ( abs ( $Z**3 + ( $Z - 1 )* $Z - $Z ) < 4 && ($iter < $maxiter) ) {
...
sub plotpixel {
	...
	if ( 1 == 2 ) { 
		...
		} elsif ( $iter >= 4 && $iter <= 30  ) {
			$iter = 129 ;

		} else { 
			$iter = 35 ; ## +52 - not bad. 62 good too. 95 good.  
		}
